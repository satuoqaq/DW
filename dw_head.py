...
from mmdet.core import distance2bbox, multi_apply, bbox_overlaps, reduce_mean, filter_scores_and_topk, \
    select_single_mlvl, bbox2distance
...
# 中心候选框 Anchor bag
# we first construct a bag of candidate positives for each GT object by selecting anchors near the GT center (center prior)
class CenterPrior(nn.Module):
    ...
    # 这是对一张图片的每一个特征层上的每一个gt找出点在gt内部范围内的点以及对应权重
    def forward(self, anchor_points_list, gt_bboxes, labels,
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            # 计算距离 ((point-gt)/stride - mean(x,y))^2
            distance = (((single_level_points - gt_center) / float(stride) - instance_center) ** 2)
            # 计算权重 e^(-r^2/(2*sigma^2)) 大概就是底数加了一个可以学习的sigma参数,e^(-r^2)
            center_prior = torch.exp(-distance / (2 * instance_sigma ** 2)).prod(dim=-1)
            # 这里只是软化了weight,并没有论文中说的smaller than a threshold or select the top-k nearest anchors

            center_prior_list.append(center_prior)
        center_prior_weights = torch.cat(center_prior_list, dim=0)
        # 如果不使用这个软化的权重，就是权重就是0,1这种
...
class DWHead(AnchorFreeHead):
    ...
    # 因为是继承anchor free，所以已经有了reg,cls分支，需要添加center_ness以及reg_refine分支
    ...
    def forward_single(self, x, scale, stride):
        ...
        if self.with_reg_refine:
            ...
            # 获取特征图的point
            points = self.prior_generator.single_level_grid_priors((h, w), self.strides.index(stride), dtype=x.dtype,
                                                                   device=x.device)
            # 重复batch次
            points = points.repeat(b, 1)
            # 将ltrb转化为特征图上的bboxes坐标(x1,y1,x2,y2) 并且化为b,c,h,w，注意这里是实际值
            ...
            # 得到新的预测结果
            bbox_pred = F.relu(
                bbox2distance(points, bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4)).reshape(b, h, w, 4).permute(0, 3, 1,
                                                                                                                2).contiguous())
    ...
    def _loss_single(self, cls_score, objectness, reg_loss, gt_labels,
        ...
        # 算是一个trick？没有使用论文中所说的iou，而是使用的G-IOU loss的值来代替iou
        # p_loc 就是论文中的IOU
        ...
        # 正样本函数权重 W_pos = e^(u*t)*t*soft_prior_norm，这里距离相关的软权重并不是完全归一化，权重稍微加大了，也是个trick?
        p_pos_weight = (torch.exp(5 * p_pos) * p_pos * center_prior_weights) / (
                torch.exp(3 * p_pos) * p_pos * center_prior_weights).sum(0, keepdim=True).clamp(min=EPS)
    ...
    def loss(self,
        ...
        # concat multi-level features to images
        ...
        cls_loss_list, loc_loss_list, cls_neg_loss_list, neg_avg_factor_list = multi_apply(self._loss_single,
                                                                                           cls_scores,
    ...
    def _get_target_single(self, gt_bboxes, points):
        ...
        if num_gts:
            ...
            inside_gt_bbox_mask = bbox_targets.new_zeros((num_points, num_gts),
                                                         dtype=torch.bool)
        # mask表示点在gt内部,bbox_target表示回归目标
